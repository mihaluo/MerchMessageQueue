...
using ISubscriber = MarchMessageQueue.Subscriber.ISubscriber;
...
<TABLE>
 ...
 <AwaitVerifyDay>0</AwaitVerifyDay>
            ...
            string key = DateTime.Today + "Order";
            HashEntry[] hashEntries = new HashEntry[500000];
            for (int i = 0; i < hashEntries.Length; i++)
            {
                hashEntries[i] = new HashEntry(Guid.NewGuid().ToString(), orderXml);
            }

            database.HashSet(key + 1, hashEntries);
            database.HashSet(key + 2, hashEntries);
            database.HashSet(key, hashEntries);

            return;
            IPublisher publisher = IocManager.Instance.Resolve<IPublisher>();

            var consumeType = typeof(SayHelloConsume);


            ISubscriber subscriber = new KafkaGeneralSubscriber();
        ...
        private static void RabbitMqTest()
        {
            ...
            IRetrSubscriber retrySubscriber = new RabbitMqReTrySubscriber();
...
<TABLE>
 ...
 <AwaitVerifyDay>0</AwaitVerifyDay>
            ...
            for (int i = 0; i < 10000; i++)
            {
                IPublisher rabbitMqPublish = new RabbitMqPublish();
                rabbitMqPublish.Publish(new SayHelloMessage
                {
